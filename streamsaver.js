(function () {
  const baseUrl = (window.__M3U8_DOWNLOADER_BASE_URL || '.').replace(/\/$/, '');
  ...
  function createWriteStream(filename) {
    ...
    if (!useBlobFallback) {
      middleTransporter = middleTransporter || (window.isSecureContext
        ? makeIframe(window.streamSaver.middleTransporterUrl)
        : makePopup(window.streamSaver.middleTransporterUrl));
  ...
  window.streamSaver = {
    createWriteStream,
    middleTransporterUrl: `${baseUrl}/mitm.html`
  };
})();
